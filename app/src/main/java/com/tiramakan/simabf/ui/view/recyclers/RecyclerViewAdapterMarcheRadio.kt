 ...
 * specified [RecyclerViewAdapterMarcheRadio].
 * TODO: Replace the implementation with code for your data type.
 */
class RecyclerViewAdapterMarcheRadio(private val context: Context, private var mValues: ArrayList<MarcheUIModel>?, private val clickListener: VHItem.ClickListener) : SelectableAdapter<RecyclerViewAdapterMarcheRadio.VHItem>() {

    val selectedItem: MarcheUIModel?
        get() {
    ...
    fun setItems(items: ArrayList<MarcheUIModel>) {
        this.mValues = items
    ...
    fun getPositionByName(marcheName: String): Int {
        ...
        for (marcheModel in mValues!!) {
            if (marcheModel.name == marcheName)
                break
            index++
        }
        return index
