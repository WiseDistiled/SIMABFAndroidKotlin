...
import com.tiramakan.simabf.ui.view.recyclers.RecyclerViewAdapterTypeOffreRadio
...
class ChoiceOfTypeOffre : BaseFragment(), RecyclerViewAdapterTypeOffreRadio.VHItem.ClickListener {
    private lateinit var mRecyclerView: RecyclerView
    private lateinit var adapter: RecyclerViewAdapterTypeOffreRadio
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
        ...
        val view = inflater.inflate(R.layout.typeoffre_item_list_radio, container, false)
        ...
        adapter = RecyclerViewAdapterTypeOffreRadio(requireContext(), TypeOffreUIModel.getList(), this)
        mRecyclerView.adapter = adapter
        val title = "Choix Type Offre "

        myParent.setTitle(title)
        val typeOffreChoisi = mybundle.getString(Constants.Extra.TYPE_OFFRE_TAG).toString()?:""
        if (!typeOffreChoisi.equals("null"))
            setSelectedItem(typeOffreChoisi)
        return view
    ...
    private fun validate() {
        if (adapter.selectedItem != null) {
            mybundle.putString(Constants.Extra.TYPE_OFFRE_TAG, adapter.selectedItem?.name.toString())
            ...
            bus.post("aucun type d'Offre sélectionné ")
    }
