 ...
 * specified [RecyclerViewAdapterProduitRadio].
 * TODO: Replace the implementation with code for your data type.
 */
class RecyclerViewAdapterProduitRadio(private val context: Context, private val mValues: ArrayList<ProduitUIModel>, private val clickListener: VHItem.ClickListener) : SelectableAdapter<RecyclerViewAdapterProduitRadio.VHItem>() {

    val selectedItem: ProduitUIModel?
        get() {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VHItem {
        ...
        val itemLayoutView = LayoutInflater.from(parent.context).inflate(
                R.layout.produit_item_radio, null)
    ...
    fun getPositionByName(marcheName: String): Int {
        var index = 0
        for (produitUIModel in mValues) {
            if (produitUIModel.name == marcheName)
                break
            index++
        }
        return index
