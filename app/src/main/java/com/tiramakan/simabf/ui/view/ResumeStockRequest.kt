 ...
 * Created by tiramakan on 17/01/2016.
 */
class ResumeStockRequest : BaseFragment() {
    ...
    private fun sendRequest() {
        try {
            ...
            val depot = realm.where(Depot::class.java)?.equalTo("nom", depotChoisi)?.findFirst()

            bus.post("Envoie de la requête de stock")
            if ((produit != null) && (depot != null) ) {
                serviceProvider.service.sendStockRequest(StockRequestModel(produit.code, depot.code).request)
    ...
    protected fun setSubTitleByList(view: View) {
        ...
        subtitleString = subtitleString +  "Dépot choisi : " + depotChoisi
        subtitleString = subtitleString + System.getProperty("line.separator")!!
