 ...
 */
class ListOffersVenteUI : BaseFragment(), RecyclerViewAdapterOffer.VHItem.ClickListener {
    ...
    private fun sendOffers() {
        val offers = realm.let { OffersToUIList.getRealmOffersNotSended(it) }
        for (offer in offers) {
            try {
                serviceProvider.service.sendOfferVente(offer)
    ...
    override fun onCreateContextMenu(menu: ContextMenu, v: View, menuInfo: ContextMenu.ContextMenuInfo?) {
        ...
        if (v.id == R.id.offerUIVenteRecycler) {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.offre_vente_ui, null)
        val recyclerView = view.findViewById<View>(R.id.offerUIVenteRecycler) as ContextMenuRecyclerView
        recyclerView.layoutManager = LinearLayoutManager(context)
        adapter = context?.let { RecyclerViewAdapterOffer(it, OffersToUIList.getListFromDBNotSended(realm), this, myPreferences.defaultCurrency) }!!
            ...
            R.id.action_edit -> {
                if (adapter.getElementAt(info.position) != null) {
                    ...
                    if (offre !== null) {
                        ...
                        bundle.putBundle("offreVenteToUIBundle", bundleRequestModelList)
                        val editOffre : Fragment
                            editOffre = SaisieOffreVenteUI()
