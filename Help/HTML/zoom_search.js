...
function ZoomInitSearch()
{
	...
	if (ToLowerSearchWords == 1)
		queryForSearch = query.toLowerCase(Locale.FRANCE);
	else
				...
				for (fieldnum = 0; fieldnum < NumMetaFields && !IsFiltered; fieldnum++)
				{
					...
					if (meta_query[fieldnum] !== "" && IsAnyDropdown == false)
					{
						...
						else if (metafields[fieldnum][METAFIELD_TYPE] == METAFIELD_TYPE_TEXT)
						{
							...
							{															
								if (GetMetaValues(i, fieldnum).toLowerCase(Locale.FRANCE).indexOf(meta_query[fieldnum].toLowerCase(Locale.FRANCE)) == -1)
									IsFiltered = true;
							}
							else
							{
								if (wordcasecmp(GetMetaValues(i, fieldnum).toLowerCase(Locale.FRANCE), meta_query[fieldnum].toLowerCase(Locale.FRANCE)) == -1)
									IsFiltered = true;
							}
						}						
...
function ZoomShowResults()
{
	...
	while (arrayline < matches && arrayline < result_limit) {
		...
		if (PdfHighlight == 1)
		{
			if (urlLink.toLowerCase(Locale.FRANCE).indexOf(".pdf") != -1)
